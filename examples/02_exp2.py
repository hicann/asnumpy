...
def create_arrays(shape: tuple[int, ...], dtype: np.dtype):
    ...
    # NumPy baseline data
    # For exp2, control input range to avoid numerical overflow
    m1_np = np.random.uniform(-10, 10, shape).astype(dtype)
...
def bench_exp2(exp2_func, m1, warmup: int, iterations: int, is_npu: bool = False) -> list:
    """
    Benchmark function for exp2 operation.
...
def run_test_case(
    ...
    try:
        ...
        result_asnp = ap.exp2(m1_asnp).to_numpy()
        result_np = np.exp2(m1_np)
...
if __name__ == "__main__":
    ...
    logger.info("Test operation: exp2 (base-2 exponential)")
    ...
    logger.info("  • Input data range limited to [-10, 10] to avoid numerical overflow")
