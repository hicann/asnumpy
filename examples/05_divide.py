...
def create_arrays(shape: tuple[int, ...], dtype: np.dtype):
    ...
    # NumPy baseline data
    # Dividend: random normal distribution
    m1_np = np.random.normal(0, 1, shape).astype(dtype)
    # Divisor: random uniform, with small offset to avoid division by zero
...
def bench_divide(divide_func, m1, m2, warmup: int, iterations: int,
                 is_npu: bool = False) -> list:
    """
    Benchmark function for divide operation.
...
if __name__ == "__main__":
    ...
    logger.info("Test operation: divide (element-wise division)")
    ...
    logger.info("  • Divisor range limited to [0.5, 2.0] to avoid division by zero")
