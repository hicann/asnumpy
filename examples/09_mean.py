...
def bench_mean(mean_func, m1, warmup: int, iterations: int, is_npu: bool = False) -> list:
    """
    Benchmark function for mean operation.
...
def run_test_case(
    ...
    try:
        ...
        result_asnp = ap.mean(m1_asnp)
        result_np = np.mean(m1_np)

        # mean(axis=None) returns scalar, compare directly
        abs_diff = abs(result_asnp - result_np)
        rel_diff = abs_diff / abs(result_np) if abs(result_np) > 0 else abs_diff
...
if __name__ == "__main__":
    ...
    logger.info("Test operation: mean (arithmetic mean)")
    ...
    logger.info("  • mean(axis=None) computes arithmetic mean of all elements")
