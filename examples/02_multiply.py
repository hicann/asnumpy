...
def main():
    ...
    print("Performing multiplication with NumPy (CPU)...")
    start_np = time.time()
    np_result = np.multiply(np_a, np_b)
    end_np = time.time()
    
    print("Performing multiplication with asnumpy (GPU/Custom Device)...")
    start_ap = time.time()
    ap_result = ap.multiply(ap_a, ap_b)
