...
@testing.numpy_asnumpy_allclose(rtol=1e-5, atol=1e-8)
def test_asarray_dtype_conversion(xp):
    """Test asarray with dtype mismatch performs conversion."""
    numpy.random.seed(42)
    data = numpy.random.uniform(-5.0, 5.0, size=(3, 4)).astype(numpy.float64)
    if xp is numpy:
        a = numpy.asarray(data, dtype=numpy.float32)
    else:
        a = xp.asarray(data, dtype=numpy.float32)
    return a
