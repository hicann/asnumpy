...
@testing.numpy_asnumpy_allclose(rtol=1e-5, atol=1e-8)
def test_array_dtype_conversion(xp):
    """Test array() converts dtype when input ndarray has different dtype."""
    numpy.random.seed(42)
    data = numpy.random.uniform(-5.0, 5.0, size=(3, 4)).astype(numpy.float64)
    if xp is numpy:
        a = numpy.array(data)
        return numpy.array(a, dtype=numpy.float32)
    else:
        a = xp.ndarray.from_numpy(data)
        return xp.array(a, dtype=numpy.float32)


@testing.numpy_asnumpy_allclose(rtol=1e-5, atol=1e-8)
def test_array_no_copy(xp):
    """Test array() with copy=False on existing ndarray of same dtype."""
    numpy.random.seed(42)
    data = numpy.random.uniform(-5.0, 5.0, size=(3,)).astype(numpy.float32)
    if xp is numpy:
        a = numpy.array(data)
        return numpy.array(a, copy=False)
    else:
        a = xp.ndarray.from_numpy(data)
        return xp.array(a, copy=False)
